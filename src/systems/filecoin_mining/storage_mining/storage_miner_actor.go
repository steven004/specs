...
func (st *StorageMinerActorState_I) _shouldChallenge(rt Runtime, networkPower block.StoragePower) bool {
	minChallengePeriod := st._computeMinChallengePeriod(rt, networkPower)
	return st.ChallengeStatus().ShouldChallenge(rt.CurrEpoch(), minChallengePeriod)
}

func (st *StorageMinerActorState_I) _computeMinChallengePeriod(rt Runtime, networkPower block.StoragePower) block.ChainEpoch {
	// TODO define function here
	fractionPower := st.SectorTable().ActivePower() / networkPower
	panic(fractionPower)

	var minChallengePeriod block.ChainEpoch
	return minChallengePeriod
}

func (a *StorageMinerActorCode_I) _shouldChallenge(rt Runtime, networkPower block.StoragePower) bool {
	h, st := a.State(rt)
	ret := st._shouldChallenge(rt, networkPower)
	Release(rt, h, st)
	return ret
}

// called by CronActor to notify StorageMiner of PoSt Surprise Challenge
func (a *StorageMinerActorCode_I) NotifyOfPoStSurpriseChallenge(rt Runtime, networkPower block.StoragePower) InvocOutput {
